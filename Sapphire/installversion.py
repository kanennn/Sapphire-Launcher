...
def addinstalledmark(version, workingDir):
    if os.path.exists(os.path.join(workingDir, "data", "installedversions.dat")):
        with open(
            ...
            instversions = pickle.load(instversionsfile)
        instversions.append(version)
        with open(
            os.path.join(workingDir, "data", "installedversions.dat"), "wb"
        ) as instversionsfile:
            pickle.dump(instversions, instversionsfile)
    else:
        with open(
            os.path.join(workingDir, "data", "installedversions.dat"), "wb"
        ) as instversionsfile:
...
def writeassetindex(assetindexurl, versionjson, workingDir, installdir):
    ...
    os.makedirs(
        os.path.join(workingDir, "game", installdir, "assets", "indexes"), exist_ok=True
    )
    with open(
        os.path.join(
            workingDir,
...
def getversionjson(version, workingDir, resourceDir):
    ...
    with open(
        os.path.join(resourceDir, "resources", "version-json-downloads.json"), "r"
    ) as versionDict:
...
def downloadassets(combineddata):
    os.makedirs(
        os.path.join(
            combineddata[2], combineddata[1], os.path.dirname(combineddata[0][1])
        ),
        exist_ok=True,
    )
    with open(
        os.path.join(combineddata[2], combineddata[1], combineddata[0][1]), "wb"
    ) as directory:
...
def writeinstalldata(versionjson, installversion, installdir, workingDir):
    ...
    with open(
        os.path.join(
            workingDir, "game", installdir, "installdata_{}.dat".format(installversion)
        ),
        "wb",
    ) as installdatafile:
...
def downloadlibraries(url, installdir, workingDir):
    ...
    for i in enumerate(url):
        ...
        with open(
            os.path.join(workingDir, installdir, "libraries", writtenfile), "wb"
        ) as directory:
...
def getllibraryurls(versionjson):
    ...
    [
        jsonurls.append(u["downloads"]["artifact"]["url"])
        for u in jsondownloads
        if u["downloads"]["artifact"]["url"] not in jsonurls
        and "lwjgl" not in u["downloads"]["artifact"]["url"]
        and "log4j" not in u["downloads"]["artifact"]["url"]
    ]
...
def getasseturls(assetindex):
    ...
    for i in objects:
        ...
        downloadurls.append(
            [
                "https://resources.download.minecraft.net/{}".format(hashprefixed),
                os.path.join("assets", "objects", hashprefixed),
            ]
        )
    return downloadurls
