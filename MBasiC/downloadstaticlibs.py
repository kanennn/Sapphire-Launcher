    ...
    with tempfile.TemporaryFile(mode="w+b") as filetemp:
        ...
        with zipfile.ZipFile(
            ...
            for file in zip.namelist():
                if file.startswith(
                    "zulu17.30.15-ca-jre17.0.1-macosx_aarch64/zulu-17.jre/"
                ):
                    trimfile = re.search(
                        "zulu17.30.15-ca-jre17.0.1-macosx_aarch64/zulu-17.jre/(.*)",
                        file,
                    ...
                    if file.endswith(os.sep):
                        os.makedirs(
                            os.path.join(
                                workingDir, "jre", *trimfile.split(os.pathsep)
                            ),
                            exist_ok=True,
                        )
                    else:
                    ...
                    if trimfile.endswith("java"):
                        with open(
                            os.path.join(workingDir, "data", "javapath.dat"), "wb"
                        ) as javapath:
                            pickle.dump(
                                [workingDir, "jre", *trimfile.split(os.pathsep)],
                                javapath,
...
def downloadstatics(workingDir):
    ...
    if not os.path.exists(os.path.join(workingDir, "data", "staticsinstalled.dat")):
        with open(
            os.path.join(workingDir, "data", "staticsinstalled.dat"), "wb"
        ) as staticsinst:
